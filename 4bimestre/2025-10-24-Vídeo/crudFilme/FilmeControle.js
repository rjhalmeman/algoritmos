...
function salvar() {
    ...
    if (id && titulo && diretor && dataLancamento && avaliacao) {// se tudo certo 
...
function persistirEmLocalPermanente(arquivoDestino, conteudo) {
    ...
    link.download = arquivoDestino; // Titulo do arquivo de download
...
function prepararESalvarCSV() { //gera um arquivo csv com as informações da lista. Vai enviar da memória RAM para dispositivo de armazenamento permanente.
    let tituloDoArquivoDestino = "./Filme.csv";  //define o titulo do arquivo csv
    let textoCSV = "";
     let fimDeLinha = "\n";
    for (let i = 0; i < listaFilme.length; i++) {
    ...
    persistirEmLocalPermanente(tituloDoArquivoDestino, textoCSV);
}
...
function listarMaiorQueOito() {

    let filmesBemAvaliados = [];
    for (let i = 0; i < listaFilme.length; i++) {
        const filme = listaFilme[i];
        if (filme.avaliacao > 8) {
            filmesBemAvaliados.push(filme);
        }
    }
    document.getElementById("outputSaida").innerHTML = preparaListagem(filmesBemAvaliados);
}