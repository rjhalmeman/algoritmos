...
function salvar() {
    ...
    const ehAlcolico = document.getElementById("inputEhAlcolico").checked; //true ou false 
...
// Função para mostrar os dados do bebida nos campos
function mostrarDadosbebida(bebida) {
    ...
    document.getElementById('inputEhAlcolico').checked = bebida.ehAlcolico;
...
function limparAtributos() {
    ...
    document.getElementById("inputEhAlcolico").checked = false; //desmarca o checkbox
    bloquearAtributos(true);
}
...
function prepararESalvarCSV() { //gera um arquivo csv com as informações da lista. Vai enviar da memória RAM para dispositivo de armazenamento permanente.
    ...
    const quebraDeLinha = "\n"; //quebra de linha
    for (let i = 0; i < listabebida.length; i++) {
        const linha = listabebida[i]; //variavel linha contem as informações de cada bebida
        if (i==listabebida.length-1) { //se for a última linha, não coloca quebra de linha no final
            textoCSV += linha.id + ";" + linha.nome + ";" + linha.fabricante + ";" + linha.precoEtiqueta + ";" + linha.ehAlcolico;            
        } else {
            textoCSV += linha.id + ";" + linha.nome + ";" + linha.fabricante + ";" + linha.precoEtiqueta + ";" + linha.ehAlcolico + "\n";
        }
    }
...
// Função para processar o arquivo CSV e transferir os dados para a listabebida
    ...
    leitor.onload = function (e) {
        ...
        for (let i = 0; i < linhas.length; i++) {
            ...
            if (linha) { //verifica se a linha não está vazia
                ...
                if (dados.length === 5) { //verifica se a linha tem a quantidade certa de atributos
                    // Adiciona os dados à listabebida como um objeto
                    listabebida.push({
                        id: dados[0],
                        nome: dados[1],
                        fabricante: dados[2],
