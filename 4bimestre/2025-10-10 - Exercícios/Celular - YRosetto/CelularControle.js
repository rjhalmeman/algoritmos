...
function procure() {
    ...
    if (numeroMac) { // se digitou um NumeroMac
...
function salvar() {
    ...
    if (numeroMac && modelo && fabricante && preco) {// se tudo certo 
...
function prepararESalvarCSV() { //gera um arquivo csv com as informações da lista. Vai enviar da memória RAM para dispositivo de armazenamento permanente.
    ...
    let textoCSV = "";
    for (let i = 0; i < listaCelular.length; i++) {
        const linha = listaCelular[i]; //variavel linha contem as informações de cada celular
...
// Função para processar o arquivo CSV e transferir os dados para a listaCelular
        ...
        for (let i = 0; i < linhas.length; i++) {
            ...
            if (linha) { //verifica se a linha não está vazia
                ...
                if (dados.length === 4) { //verifica os seis campos
                    // Adiciona os dados à listaCelular como um objeto
                    listaCelular.push({
                        numeroMac: dados[0],
                        modelo: dados[1],
                        fabricante: dados[2],
